...
(function ($, window, document, undefined) {
    ...
    $.extend({
         ...
         *   @return number
         */
        round: function (srcValuef, iCount) {
         ...
         * value 需要匹配的值
         */
        getValueOfArray: function (array, key, view, value) {
            if ($.isEmpty(array)) {
                return null;
            }
         ...
         * @param object
         */
        showAlert: function (content, title, width, level, callback, object) {
            if ($.isEmpty(object)) {
                object = window;
            }
         ...
         * @param object
         */
        showConfirm: function (content, title, width, buttons, object) {
            if ($.isEmpty(object)) {
                object = window;
            }
         ...
         * @param object
         */
        showDialog: function (title, url, width, height, params, buttons, object) {
            if ($.isEmpty(object)) {
                object = window;
            }
        ...
        */
        getTopPage: function (page) {
            ...
            while (!$.isEmpty(page) && page.find("#base").length > 0 && cnt < 10) {
                page = page.parent;
        ...
        initPage: function () {
            ...
            _dialog = $.isEmpty(_dialogParams) ? null : _dialogParams.dialog;
    ...
    $.extend({
        //创建搜索工具栏
        createSearchToolBar: function (options, element) {
            ...
            if (options.search) {
                ...
                searchBtnDiv.off('click').on('click', function () {
                    if (!$.isEmpty(options.searchFunc)) {
                        options.searchFunc(1);
                    } else {
                        $.showAlert("未设置查询方法search_onclick", null, null, "danger");
                    }
                });
            ...
            leftDiv.append(form);
            $(element).append(toolBarDiv.append(leftDiv));
            ...
            if (form.find('div').length > 0) {
                ...
                if (options.combineSearch) {
                    ...
                    $(element).append($('<div style="height: 10px;"></div>'));
                }
            }
        },
        //检索类别下拉框改变事件
        searchSelect_onchage: function (src) {
        ...
        //创建组合查询
        createCombineSearch: function (element) {
            ...
            if (!_tableOptions.search) {
                ...
                $('#search_btn_a').off('click').on('click', function () {
                    if (!$.isEmpty(_tableOptions.searchFunc)) {
        ...
        //创建表头
        createThead: function (options, element) {
            ...
            if ($.isEmpty(element)) {
                ...
                $(element).empty().append(thead);
            }
        },
        //绑定表格数据
        bindTbody: function (data, element) {
        ...
        //创建分页
        createPagination: function (pageSize, pageNo, totalSize, pageList, element) {
            var dataTable;
            ...
            if ($('#table_pagination').length <= 0) {
                ...
                $('#paginate_select').empty().off('change').on('change', function () {
                    if (!$.isEmpty(_tableOptions.searchFunc)) {
                        _tableOptions.searchFunc($.getCurrentPageIndex());
                    } else {
                        $.showAlert("未设置查询方法search_onclick", null, null, "danger");
                    }
                });
                ...
                $('#paginate_previous').off('click').on('click', function () {
                    if (!$.isEmpty(_tableOptions.searchFunc)) {
                        _tableOptions.searchFunc($.getCurrentPageIndex() - 1 > 0 ? $.getCurrentPageIndex() - 1 : 1);
                    } else {
                        $.showAlert("未设置查询方法search_onclick", null, null, "danger");
                    }
                });
                ...
                $('#paginate_next').off('click').on('click', function () {
                    if (!$.isEmpty(_tableOptions.searchFunc)) {
                        _tableOptions.searchFunc($.getCurrentPageIndex() + 1 < parseInt($(paginate_pages).text(), 10) ? $.getCurrentPageIndex() + 1 : parseInt($(paginate_pages).text(), 10));
                    } else {
                        $.showAlert("未设置查询方法search_onclick", null, null, "danger");
                    }
                });
     ...
     */
    var table = function (ele, options, data) {
        //调用对象
        this.$element = ele;
        //默认选项
        this._defaultOptions = {
            add: true,                   //新增，默认true
            search: true,                //搜索，默认true
            searchFunc: undefined,        //搜索按钮点击事件，默认search_onclick
        ...
        //表格数据
        this._data = data;
        _tableOptions = this._options;
    };

    table.prototype = {
        init: function () {
            //创建搜索工具栏
            $.createSearchToolBar(this._options, this.$element);

            //创建表格
            $(this.$element).append('<div class="panel panel-default panel-table" style="position: relative"><table id="dataTable" class="table table-striped table-hover"></table></div>');

            //创建表头
            $.createThead(this._options);
            //绑定表格数据
            $.bindTbody(this._data);
            ...
            if (this._options) {
                ...
                if (!$.isEmpty(this._data)) {
                    pageSize = $.nullToObject(this._data.pageSize, 0);
                    pageNo = $.nullToObject(this._data.pageNo, 1);
                    totalSize = $.nullToObject(this._data.totalSize, 0);
                }

                $.createPagination(pageSize, pageNo, totalSize, this._options.pageList);
            }
        }
    };

    $.fn.createTable = function (options, data) {
        new table(this, options, data).init();
        return this;
    };
